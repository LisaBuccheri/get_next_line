...
/*   Updated: 2021/09/28 12:58:21 by lbuccher         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*mem_buffer[FOPEN_MAX];
	char		*buff_str;
	char		*r_value;
	int			read_val;
	...
	read_val = 1;
	r_value = NULL;
	while (!(ft_strchr (mem_buffer[fd], '\n')) && read_val > 0)
	{
		buff_str = ft_calloc((BUFFER_SIZE + 1), sizeof(char));
		if (!buff_str)
			return (NULL);
		read_val = read(fd, buff_str, BUFFER_SIZE);
		mem_buffer[fd] = ft_strjoin(mem_buffer[fd], buff_str);
		free(buff_str);
	}
	if (!mem_buffer[fd])
		return (NULL);
	r_value = get_return_value(mem_buffer[fd]);
	mem_buffer[fd] = get_next_buffer(mem_buffer[fd]);
	return (r_value);
}
